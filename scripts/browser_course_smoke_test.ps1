...
try {
  ...
  Invoke-PwCli eval "() => { if (document.querySelectorAll('.module').length !== 5) throw new Error('module library incomplete'); const routes = [...document.querySelectorAll('[data-module]')].map(link => link.getAttribute('href')); if (!routes.includes('learning-analytics/scene/index.html') || !routes.includes('edm-patterns/scene/index.html')) throw new Error('application lesson routes missing'); return 'course-library-ready'; }" | Out-Null
