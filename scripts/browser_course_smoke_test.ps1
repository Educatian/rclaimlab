...
$session = "rclaimlab-course-smoke"
$server = $null
...
try {
  ...
  Invoke-PwCli eval "() => { const catalog = window.RCLAIMLAB_CATALOG; const modules = document.querySelectorAll('.module'); if (!catalog || modules.length !== catalog.modules.length || modules.length !== 10) throw new Error('module library and catalog disagree'); const routes = [...document.querySelectorAll('[data-module]')].map(link => link.getAttribute('href')); const required = ['statistics-distribution/scene/index.html','statistics-bootstrap/scene/index.html','statistics-categories/scene/index.html','learning-analytics/scene/index.html','edm-patterns/scene/index.html']; if (required.some(route => !routes.includes(route))) throw new Error('reference lesson routes missing'); return 'course-library-ready'; }" | Out-Null
  ...
  Invoke-PwCli screenshot --filename="output/playwright/rclaimlab-course-home-desktop.png" | Out-Null
  Invoke-PwCli click "button[data-filter='Statistics']" | Out-Null
  Invoke-PwCli eval "() => { const expected = window.RCLAIMLAB_CATALOG.modules.filter(module => module.track === 'Statistics').length; const visible = [...document.querySelectorAll('.module')].filter(x => x.dataset.hidden !== 'true'); if (visible.length !== expected || expected !== 7) throw new Error('statistics filter failed'); return 'filter-ok'; }" | Out-Null
  Invoke-PwCli eval "() => { const catalog = window.RCLAIMLAB_CATALOG; const key = 'rclaimlab:' + catalog.course_id + ':progress:v1'; localStorage.setItem(key, JSON.stringify({last:'statistics-pca','statistics-pca':true})); window.dispatchEvent(new Event('pageshow')); const module = document.querySelector('[data-module=statistics-pca]').closest('.module'); if (!module.querySelector('.module-status').textContent.includes('Completed')) throw new Error('receipt-backed progress did not persist'); return 'progress-ok'; }" | Out-Null
  ...
  Invoke-PwCli screenshot --filename="output/playwright/rclaimlab-course-home-mobile.png" | Out-Null
  Invoke-PwCli close | Out-Null
  Write-Output "R-ClaimLab course-home browser smoke test passed."
} finally {
