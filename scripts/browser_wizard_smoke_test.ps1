...
try {
  ...
  Invoke-PwCli eval "() => { const text = document.body.innerText; if (!text.includes('Explore numeric dimensions') || !text.includes('Recommended')) throw new Error('question-first descriptive recommendation missing'); if (document.querySelector('#analysis')?.value !== 'data_view') throw new Error('explore intent must start with data_view'); if (document.documentElement.scrollWidth > document.documentElement.clientWidth + 1) throw new Error('desktop horizontal overflow'); return 'question-first-recommendation-ready'; }" | Out-Null
  Invoke-PwCli run-code "async page => { await page.selectOption('#intent', 'reduce'); await page.waitForFunction(() => document.querySelector('#analysis')?.value === 'prcomp'); }" | Out-Null
  Invoke-PwCli eval "() => { const text = document.querySelector('#analysis_recommendations').innerText; if (!text.includes('Principal component analysis') || !text.includes('Recommended')) throw new Error('reduce intent PCA recommendation missing'); return 'reduce-recommendation-ready'; }" | Out-Null
  Invoke-PwCli run-code "async page => { await page.selectOption('#outcome', 'transfer_score'); await page.selectOption('#intent', 'explain'); await page.waitForFunction(() => document.querySelector('#analysis')?.value === 'lm' && ![...document.querySelector('#dimensions').selectedOptions].some(option => option.value === 'transfer_score')); }" | Out-Null
  Invoke-PwCli run-code "async page => { await page.selectOption('#grouping', 'learner_id'); await page.waitForFunction(() => document.querySelector('#analysis')?.value === 'data_view' && document.querySelector('#analysis_recommendations')?.innerText.includes('not recommended because grouped')); }" | Out-Null
  Invoke-PwCli eval "() => { const text = document.querySelector('#analysis_recommendations').innerText; if (!text.includes('Use a grouped or longitudinal model')) throw new Error('dependence caution missing'); return 'dependence-gate-ready'; }" | Out-Null
  Invoke-PwCli run-code "async page => { await page.selectOption('#outcome', ''); await page.selectOption('#intent', 'reduce'); await page.selectOption('#grouping', 'learner_id'); await page.selectOption('#time', 'session'); await page.selectOption('#id_column', 'event_id'); await page.fill('#question', 'Which learning activity measures vary together, and which sessions have contrasting PCA scores?'); await page.fill('#unit_of_analysis', 'one synthetic learner-session summary'); await page.waitForFunction(() => document.querySelector('#analysis')?.value === 'prcomp' && ![...document.querySelector('#dimensions').selectedOptions].some(option => option.value === 'session')); }" | Out-Null
