rlearnxr_source_root <- if (exists("root", inherits = FALSE)) {
...
for (rlearnxr_source_file in rlearnxr_source_files) {
  source(file.path(rlearnxr_source_root, "R", rlearnxr_source_file), chdir = FALSE)
}
rm(rlearnxr_source_file, rlearnxr_source_files)
