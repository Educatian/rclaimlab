test_that("render_scene creates an accessible complete learning loop", {
  ...
  expect_match(html, 'id="step-guidance"', fixed = TRUE)
  expect_match(html, 'id="observation-heading">Preview observation', fixed = TRUE)
  expect_match(html, 'observationHeading.textContent = evidenceSelected ? "Selected observation" : "Preview observation"', fixed = TRUE)
  expect_match(html, 'Choose an observation in the table, 2D plot, or 3D scene.', fixed = TRUE)
  expect_match(html, 'Array.isArray(lessonSpec.misconceptions)', fixed = TRUE)
  expect_match(html, 'learningGoals.replaceChildren()', fixed = TRUE)
  ...
  expect_match(html, '.selected-tag { max-width: 100%', fixed = TRUE)
  expect_match(html, '.prediction-summary { display: flex; flex-wrap: wrap', fixed = TRUE)
  expect_match(html, '.code-toolbar { display: flex; flex-wrap: wrap', fixed = TRUE)
  expect_match(html, '.ai-meta, .repro-grid, .repro-summary, .criteria-grid { grid-template-columns: 1fr; }', fixed = TRUE)
  expect_match(html, '.step-count { flex: 1 1 100%; padding-top: 0; white-space: normal; }', fixed = TRUE)
  expect_false(grepl('text-overflow: ellipsis', html, fixed = TRUE))
  expect_false(grepl('-webkit-line-clamp', html, fixed = TRUE))
