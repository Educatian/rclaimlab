test_that("evidence adapters reject malformed method inputs", {
  ...
  expect_error(as_rlearnxr_evidence(pca, components = "PC1"), "two")
  factor_lm <- suppressWarnings(stats::lm(Species ~ Sepal.Length, data = iris))
  expect_error(as_rlearnxr_evidence(factor_lm), "numeric response")

  model <- stats::glm(am ~ wt, data = mtcars, family = stats::binomial())
  expect_error(as_rlearnxr_evidence(model, threshold = 0), "between")
  expect_error(as_rlearnxr_evidence(model, threshold = c(.4, .6)), "one number")

  set.seed(9)
  km <- stats::kmeans(iris[1:10, 1:4], centers = 2)
  expect_error(as_rlearnxr_evidence(km, data = iris[1:9, 1:4]), "rows")
  expect_error(as_rlearnxr_evidence(km, data = iris[1:10, 1:4], dimensions = "Sepal.Length"), "two")
  broken <- iris[1:10, 1:4]
  broken[1, 1] <- Inf
  expect_error(as_rlearnxr_evidence(km, data = broken), "non-finite")
})

test_that("Evidence IR validator identifies each broken link", {
  evidence <- as_rlearnxr_evidence(iris[1:5, 1:3])
  expect_output(print(evidence), "rlearnxr_evidence")
  ...
  expect_error(validate_rlearnxr_evidence(broken), "must be")
  broken <- evidence; broken$schema_version <- "old"
  expect_error(validate_rlearnxr_evidence(broken), "unsupported")
  broken <- evidence; broken$analysis$artifact_hash <- NULL
  expect_error(validate_rlearnxr_evidence(broken), "incomplete")
  broken <- evidence; broken$observations$observation_id[2] <- broken$observations$observation_id[1]
  expect_error(validate_rlearnxr_evidence(broken), "unique")
  broken <- evidence; broken$values$observation_id[1] <- "missing"
  expect_error(validate_rlearnxr_evidence(broken), "dangling")
  broken <- evidence; broken$links$evidence_id[1] <- "wrong"
  expect_error(validate_rlearnxr_evidence(broken), "synchronized")
  broken <- evidence; broken$values$value[1] <- Inf
  expect_error(validate_rlearnxr_evidence(broken), "finite")

  expect_error(read_rlearnxr_evidence(tempfile()), "not found")
  file <- tempfile(fileext = ".json")
  write_rlearnxr_evidence(evidence, file)
  expect_error(write_rlearnxr_evidence(evidence, file), "already exists")
})
...
test_that("lesson specifications enforce the educational sequence", {
  ...
  expect_output(print(lesson), "rlearnxr_lesson")
...
test_that("compiler and build methods cover overwrite and summary contracts", {
  evidence <- as_rlearnxr_evidence(iris[1:6, 1:3])
  ...
  expect_output(print(build), "rlearnxr_build")
  ...
  expect_s3_class(compile_lesson(lesson, output, overwrite = TRUE), "rlearnxr_build")
...
test_that("version 2 receipt methods reject malformed attempts", {
  ...
  expect_output(print(receipt), "rlearnxr_receipt")
