test_that("optional Shiny shell is exported without becoming a core dependency", {
  expect_true(is.function(run_rlearnxr_shiny))
  description <- utils::packageDescription("rlearnxr")
...
test_that("Shiny shell reports its optional dependency clearly", {
  ...
  expect_error(
    run_rlearnxr_shiny(launch.browser = FALSE),
...
test_that("Shiny educator app can be constructed without launching a browser", {
  ...
  app <- rlearnxr:::build_rlearnxr_shiny_app(catalog = default_course_catalog())
  expect_s3_class(app, "shiny.appobj")
  expect_true(is.function(app$serverFuncSource))
  expect_error(rlearnxr:::build_rlearnxr_shiny_app(lesson_dir = tempfile()), "does not exist")
})
